...
parser.add_argument('--word_vec', type=str, default=constants.WORD_VEC_PATH)
...
if args.hp_path:
    ...
    configs.cdims = 30
    configs.use_conv = True
    configs.num_filters = 200
    configs.kernel_size = 3
    configs.dilation_rate = [1, 2]
    configs.num_hidden_word = [100, 100]
    configs.num_hidden_char = [50]
    ...
    configs.clip_grad = 'global'
    configs.use_residual = True
    configs.use_bi_causal_conv = False
    ...
    configs.bi_causal_conv_block = 3
    configs.latent_dim = 100
...
train_iter, dev_iter = read_data.read_folder(
    args.data, args.batch_size, args.num_epoch, 0.2, random_seed=42, buffer_size=3000)
if args.checkpoints:
    model.saver.restore(
